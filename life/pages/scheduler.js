...
const SCHEDULER = {
  ...
  buildBookPlan(book, queueItem, phases, routine, rev, calRules, startDate) {
    ...
    const plan = [];
    // Always build the plan from startPage and startDate — not from current_page.
    // This means the schedule never shifts when the user logs ahead.
    // The reading log (not current_page) determines which days are marked done in the UI.
    let page = startPage;
    let date = startDate;
    const MAX = 800;
